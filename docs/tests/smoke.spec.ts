...
test.describe('docs site — page smoke', () => {
  for (const page of ALL_PAGES) {
    test(`${page.lang.toUpperCase()} ${page.rel}`, async ({ page: pw }) => {
      ...
      // rspress is an SPA with client-side MDX hydration. On
      // `domcontentloaded` the document shell is parsed but the
      // <h1> / <header> may still be appearing as React mounts the
      // page-level components — particularly on JA-locale routes
      // where the i18n re-render seems to land slightly later under
      // Chromium and Firefox (WebKit happens to consistently see the
      // hydrated DOM by DCL). Wait for both elements to attach before
      // asserting; locator.waitFor with `state: 'attached'` polls and
      // resolves the moment the node enters the DOM, so we don't pay
      // a fixed sleep on fast pages.
      await pw
        .locator('h1')
        .first()
