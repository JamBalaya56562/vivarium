...
<Page>
  <PageHero
    ...
    sub="Vivarium は MCP（Model Context Protocol）サーバを同梱しており、複数のツールでカタログ全体をエージェントから機械的に検索・取得し、新規レシピ作成や branch-fix / fix-candidate 検証も足場まで一気に組める。docs サイトを HTML スクレイプする必要はない。"
  />
  ...
  >
    <p>Vivarium MCP サーバ (<code>@aletheia-works/vivarium-mcp</code>) は{' '} <a href="https://modelcontextprotocol.io">Model Context Protocol</a>{' '} の stdio トランスポート経由で一連のツールを提供します (カタログの列挙、レシピ単体のメタデータ取得、verdict スナップショット、 エラー → レシピのマッチング、加えて新規レシピと branch-fix / fix-candidate 検証用のスキャフォルダー)。 エージェント側からカタログを検索し、特定レシピのメタデータを取得し、 Layer 2 / 3 の verdict スナップショットを読み、候補修正の検証フローを 組み立てられます。各ツールの入出力詳細は下のセクションに一覧があります。</p>
    <Callout>
...
`}</code></pre>
    <p><code>dist/index.js</code> の<strong>絶対パス</strong>を控えておきます。 MCP クライアント側の設定で、<code>node</code> 経由でこのファイルを起動する形になります。</p>
  </Section>
  ...
  >
    <p>MCP サーバの登録は、どのクライアントでも本質的に「<code>command</code>{' '} と <code>args</code> の組」を JSON で渡す形です:</p>
    <pre><code>{`{
  "mcpServers": {
    "vivarium": {
      "command": "node",
...
`}</code></pre>
    <p>この snippet を貼る場所がクライアントごとに違うだけです:</p>
    <NumberedList
      items={[
        {
  ...
  <Section
    eyebrow="// 3 · ツール一覧"
    heading="どのツールも MCP の標準 stdio 経由で呼べる。"
  >
    <ul>
      <li>
      ...
      <li>
        ...
        <code>docs/site/_data/recipe-facets.json</code>（と新しいプロジェクトのみ
        <code>docs/site/_data/projects.json</code>）に追加する placeholder 行、
    ...
    <NumberedList
      items={[
        ...
        },
        {
          ...
          body: <>修正候補が本当に reproduced → unreproduced を起こしたかをエージェントから一発で確認するフローは <code>verify_branch_fix</code> に集約されています。フル手順は <a href="/vivarium/ja/guide/compare-branch-fix">「ブランチ修正を検証する」</a>を参照。</>,
        },
      ]}
    />
  </Section>
