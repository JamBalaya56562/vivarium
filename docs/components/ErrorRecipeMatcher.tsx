...
const STRINGS: Record<Lang, Strings> = {
  en: {
    ...
    noInputBody:
      'Paste an error or stack trace above and press "Find matches".',
    scoreLabel: 'score',
  ...
  ja: {
    ...
    noInputBody:
      '上のエリアにエラーまたはスタックを貼り付けて「マッチを探す」を押す。',
    scoreLabel: 'スコア',
...
export function ErrorRecipeMatcher({ lang }: { lang: Lang }) {
  ...
  const tokens = useMemo(() => tokenise(submitted), [submitted]);
  ...
  return (
    <section className="v-erm">
      ...
      <div className="v-erm__actions">
        <button
          type="button"
          className="v-erm__btn v-erm__btn--primary"
          onClick={submit}
          disabled={!input.trim()}
        >
          {s.matchButton}
