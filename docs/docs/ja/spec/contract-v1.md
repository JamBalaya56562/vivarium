...
>（プライベートメモ）によって v1 にロックされている。現在 **リビジョン 3** ——
...
> **語彙ノート。** リビジョン 1 と 2 では verdict 値は `"pass"` / `"fail"` だった。
> 元の表現は典型的な CI の「green = good」フレームと意図的に逆向きで——「pass」は
> バグが**再現された**ことを意味した——同時にすべての読み手に毎回認知コストを
> 課していた。リビジョン 3
> （[ADR-0029](https://github.com/aletheia-works/vivarium/blob/main/_context/decisions/0029-contract-v1-verdict-rename.md)、
> プライベートメモ）で値リテラルを `"reproduced"` / `"unreproduced"` にリネームし、
> 値の名前そのものが意味と直接一致するようにした。Contract バージョンリテラル（`v1`）は
> 不変。これは
> [`AGENTS.md` § 4.11](https://github.com/aletheia-works/vivarium/blob/main/AGENTS.md)
> の「採用前破壊的変更カーブアウト」のもとで v1 内に吸収された破壊的変更だ。
...
| 3 | 2026-05-05 | [ADR-0029](https://github.com/aletheia-works/vivarium/blob/main/_context/decisions/0029-contract-v1-verdict-rename.md)（プライベートメモ） | **v1 内破壊的変更。** Verdict enum 値をリネーム: `"pass"` → `"reproduced"`、`"fail"` → `"unreproduced"`（`"pending"` は不変）。Contract バージョンリテラルは `v1` のまま。`verdict.schema.json` の enum も更新され、旧値（`"pass"` / `"fail"`）を持つスナップショットはバリデーションを通らなくなる。`#verdict` の CSS クラス名と `data-verdict` 属性値も同じくリネーム。`v2` バンプではなく v1 内吸収としたのは、[`AGENTS.md` § 4.11](https://github.com/aletheia-works/vivarium/blob/main/AGENTS.md) の「採用前破壊的変更カーブアウト」のもとでの判断。 |
