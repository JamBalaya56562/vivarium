...
キャプチャし、元のものと比較する。元の verdict が `"reproduced"`（バグが再現される）で
branch-fix の verdict が `"unreproduced"`（バグが再現されない）なら、修正は機能している。
...
（リビジョン 3 で `pass`/`fail` からリネーム——ADR-0029 によって、
値の名前そのものが意味と一致するようになり、典型的な「green CI = good」
フレームを反転させるかつての設計が解消された。）
