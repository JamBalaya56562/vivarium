...
<Page>
  ...
  >
    <p>
      ...
      の stdio トランスポート経由で一連のツールを提供します
      (カタログの列挙、レシピ単体のメタデータ取得、verdict スナップショット、
      エラー → レシピのマッチング、加えて新規レシピと
      branch-fix / fix-candidate 検証用のスキャフォルダー)。
      エージェント側からカタログを検索し、特定レシピのメタデータを取得し、
      Layer 2 / 3 の verdict スナップショットを読み、候補修正の検証フローを
      組み立てられます。各ツールの入出力詳細は下のセクションに一覧があります。
    </p>
    <Callout>
