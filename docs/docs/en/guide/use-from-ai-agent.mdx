...
<Page>
  ...
  >
    <p>
      ...
      over stdio and exposes a set of tools (catalogue enumeration,
      recipe lookup, verdict snapshots, error-to-recipe matching, plus
      scaffolders for new recipes and for branch-fix / fix-candidate
      verification). An agent can browse the catalogue, fetch metadata
      for a given recipe, pull deployed Layer 2 / 3 verdict snapshots,
      and scaffold the AI-slop verification loop end-to-end. The full
      list and per-tool inputs are in the section below.
    </p>
    <Callout>
