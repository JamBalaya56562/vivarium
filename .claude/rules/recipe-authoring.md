...
- **Pyodide runs in the shared worker; a recipe writes none of its own.**
  ...
  A worker cannot import `_shared/verdict.ts`, `loader.ts` or
  `runner.ts` — all three reach `_assets/chrome.js`, which touches
  `document` at module-evaluation time. The shared worker therefore
  imports nothing from its own directory; everything DOM-bound stays on
  the main thread.
