...
- **Pyodide belongs in a Web Worker.** Booting it and resolving a
  ...
  after. A worker cannot import `_shared/verdict.ts`, `loader.ts`,
  `runner.ts` or `path_a.ts` — they reach `_assets/chrome.js`, which
  touches `document` at module-evaluation time — so it loads Pyodide
  itself and posts results back. See `dateutil-1478/repro.worker.ts`.
