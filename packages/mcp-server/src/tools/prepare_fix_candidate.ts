  ...
  inputSchema: {
    ...
    properties: {
      slug: {
        ...
        description:
          "Existing Layer 1 recipe slug to register the fix candidate on (e.g. 'astroid-2993'). Must resolve to a Layer 1 entry in the bundled catalogue — call list_recipes if you need to discover it.",
      },
      fork_url: {
        type: 'string' as const,
        pattern: '^https?://github\\.com/[^/]+/[^/]+/?$',
        description:
          "Bare GitHub repo URL of the fork that hosts the fix branch (e.g. 'https://github.com/JamBalaya56562/astroid'). Tree / blob / pull URLs are rejected; pass the canonical clone URL.",
      },
      branch: {
        type: 'string' as const,
        description:
          "Branch name on the fork that contains the fix (e.g. 'claude/fix-astroid-2993-wVitv').",
      },
      upstream_pr: {
        type: 'string' as const,
        description:
          "Optional URL of the PR you opened from this fork branch (e.g. 'https://github.com/pylint-dev/astroid/pull/<n>' or the fork-side draft PR URL while the upstream PR is still in progress). Embedded in the generated fix-candidate.json and surfaced in the page UI.",
      },
      package: {
        type: 'string' as const,
        description:
          "Optional pip distribution name override. Defaults to the recipe's project name (e.g. 'astroid' for the astroid-2993 recipe). Override only when the project name and the pip distribution name diverge.",
      },
      purpose: {
        type: 'string' as const,
        description:
